...
class wgCaptcha {
	...
	public static function checkCaptcha($input) {
		//if ((bool) wgUsers::getDetail() || moduleUsers::isLogged()) return true;
	...
	function stringGen() {
		...
		$c = (count($arr)-1);
		$r = rand(0, $c);
		$this->CaptchaString = $arr[$r];
	}

	function makeCaptcha() {
		$imagelength = $this->Length * 25 + 20;
		$imageheight = 50;
		$image = imagecreate ( $imagelength, $imageheight );
		//$bgcolor = imagecolorallocate($image, 255, 10, 10);
		...
		for($i = 0; $i < strlen ( $this->CaptchaString ); $i ++) {
			imagettftext($image, 14, mt_rand ( - 10, 10 ), $i * 25 + 10, mt_rand( 30, 40 ), $stringcolor, $this->getRandFont (), $this->CaptchaString {$i} );
		}
