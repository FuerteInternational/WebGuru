 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/blackhat_pages
 * @author       Ondrej Rafaj ()
...
class BlackhatPagesModel extends BaseBlackhatPagesModel {
	
	
	// --------------------- Predefined functions for BlackhatPages ---------------------
	...
	/*
	public static function getOneSelfData($idBlackhatPages) {
		$id = (int) $idBlackhatPages;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new BlackhatPagesModel();
	}
	//*/
