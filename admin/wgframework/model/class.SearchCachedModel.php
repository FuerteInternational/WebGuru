 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/search_cached
 * @author       Ondrej Rafaj ()
...
class SearchCachedModel extends BaseSearchCachedModel {
	
	
	// --------------------- Predefined functions for SearchCached ---------------------
	...
	/*
	public static function getOneSelfData($idSearchCached) {
		$id = (int) $idSearchCached;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new SearchCachedModel();
	}
	//*/
