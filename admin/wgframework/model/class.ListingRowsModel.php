 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/listing_rows
 * @author       Ondrej Rafaj ()
...
class ListingRowsModel extends BaseListingRowsModel {
	
	
	// --------------------- Predefined functions for ListingRows ---------------------
	...
	/*
	public static function getOneSelfData($idListingRows) {
		$id = (int) $idListingRows;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new ListingRowsModel();
	}
	//*/
