 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/poll_questions
 * @author       Ondrej Rafaj ()
...
class PollQuestionsModel extends BasePollQuestionsModel {
	
	
	// --------------------- Predefined functions for PollQuestions ---------------------
	...
	/*
	public static function getOneSelfData($idPollQuestions) {
		$id = (int) $idPollQuestions;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new PollQuestionsModel();
	}
	//*/
