 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/tracker_image
 * @author       Ondrej Rafaj ()
...
class TrackerImageModel extends BaseTrackerImageModel {
	
	
	// --------------------- Predefined functions for TrackerImage ---------------------
	...
	/*
	public static function getOneSelfData($idTrackerImage) {
		$id = (int) $idTrackerImage;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new TrackerImageModel();
	}
	//*/
