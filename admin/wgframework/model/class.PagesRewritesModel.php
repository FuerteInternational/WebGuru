 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/pages_rewrites
 * @author       Ondrej Rafaj ()
...
class PagesRewritesModel extends BasePagesRewritesModel {
	
	
	// --------------------- Predefined functions for PagesRewrites ---------------------
	...
	/*
	public static function getOneSelfData($idPagesRewrites) {
		$id = (int) $idPagesRewrites;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new PagesRewritesModel();
	}
	//*/
