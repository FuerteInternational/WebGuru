 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/newsletter_mails
 * @author       Ondrej Rafaj ()
...
class NewsletterMailsModel extends BaseNewsletterMailsModel {
	
	
	// --------------------- Predefined functions for NewsletterMails ---------------------
	...
	/*
	public static function getOneSelfData($idNewsletterMails) {
		$id = (int) $idNewsletterMails;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new NewsletterMailsModel();
	}
	//*/
