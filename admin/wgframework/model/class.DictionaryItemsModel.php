 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/dictionary_items
 * @author       Ondrej Rafaj ()
...
class DictionaryItemsModel extends BaseDictionaryItemsModel {
	
	
	// --------------------- Predefined functions for DictionaryItems ---------------------
	...
	/*
	public static function getOneSelfData($idDictionaryItems) {
		$id = (int) $idDictionaryItems;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new DictionaryItemsModel();
	}
	//*/
