 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/discuss_posts
 * @author       Ondrej Rafaj ()
...
class DiscussPostsModel extends BaseDiscussPostsModel {
	
	
	// --------------------- Predefined functions for DiscussPosts ---------------------
	...
	/*
	public static function getOneSelfData($idDiscussPosts) {
		$id = (int) $idDiscussPosts;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new DiscussPostsModel();
	}
	//*/
