 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/generate_info
 * @author       Ondrej Rafaj ()
...
class GenerateInfoModel extends BaseGenerateInfoModel {
	
	
	// --------------------- Predefined functions for GenerateInfo ---------------------
	...
	/*
	public static function getOneSelfData($idGenerateInfo) {
		$id = (int) $idGenerateInfo;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new GenerateInfoModel();
	}
	//*/
