 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/dictionary_listings
 * @author       Ondrej Rafaj ()
...
class DictionaryListingsModel extends BaseDictionaryListingsModel {
	
	
	// --------------------- Predefined functions for DictionaryListings ---------------------
	...
	/*
	public static function getOneSelfData($idDictionaryListings) {
		$id = (int) $idDictionaryListings;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new DictionaryListingsModel();
	}
	//*/
