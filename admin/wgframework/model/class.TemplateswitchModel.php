 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/templateswitch
 * @author       Ondrej Rafaj ()
...
class TemplateswitchModel extends BaseTemplateswitchModel {
	
	
	// --------------------- Predefined functions for Templateswitch ---------------------
	...
	public static function getTemplateByIdentifier($identifier) {
		...
		else return new TemplateswitchModel();
	}
...
/*
	...
	/*
	public static function getOneSelfData($idTemplateswitch) {
		$id = (int) $idTemplateswitch;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
