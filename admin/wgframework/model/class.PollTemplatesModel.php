 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/poll_templates
 * @author       Ondrej Rafaj ()
...
class PollTemplatesModel extends BasePollTemplatesModel {
	
	
	// --------------------- Predefined functions for PollTemplates ---------------------
	...
	public static function getTemplateByIdentifier($identifier) {
		...
		else return new PollTemplatesModel();
	}
	//*/
...
/*
	...
	/*
	public static function getOneSelfData($idPollTemplates) {
		$id = (int) $idPollTemplates;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
