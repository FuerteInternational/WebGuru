 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/pages_sitemap
 * @author       Ondrej Rafaj ()
...
class PagesSitemapModel extends BasePagesSitemapModel {
	
	
	// --------------------- Predefined functions for PagesSitemap ---------------------
	...
	/*
	public static function getOneSelfData($idPagesSitemap) {
		$id = (int) $idPagesSitemap;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new PagesSitemapModel();
	}
	//*/
