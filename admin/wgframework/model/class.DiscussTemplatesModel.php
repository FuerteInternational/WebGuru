 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/discuss_templates
 * @author       Ondrej Rafaj ()
...
class DiscussTemplatesModel extends BaseDiscussTemplatesModel {
	
	
	// --------------------- Predefined functions for DiscussTemplates ---------------------
	...
	public static function getTemplateByIdentifier($identifier) {
		...
		else return new DiscussTemplatesModel();
	}
	//*/
...
/*
	...
	/*
	public static function getOneSelfData($idDiscussTemplates) {
		$id = (int) $idDiscussTemplates;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
