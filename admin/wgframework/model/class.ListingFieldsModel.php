 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/listing_fields
 * @author       Ondrej Rafaj ()
...
class ListingFieldsModel extends BaseListingFieldsModel {
	
	
	// --------------------- Predefined functions for ListingFields ---------------------
	...
	/*
	public static function getOneSelfData($idListingFields) {
		$id = (int) $idListingFields;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new ListingFieldsModel();
	}
	//*/
