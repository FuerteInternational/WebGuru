 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/tinymce_images
 * @author       Ondrej Rafaj ()
...
class TinymceImagesModel extends BaseTinymceImagesModel {
	
	
	// --------------------- Predefined functions for TinymceImages ---------------------
	...
	/*
	public static function getOneSelfData($idTinymceImages) {
		$id = (int) $idTinymceImages;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new TinymceImagesModel();
	}
	//*/
