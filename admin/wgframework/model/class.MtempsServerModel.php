 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/mtemps_server
 * @author       Ondrej Rafaj ()
...
class MtempsServerModel extends BaseMtempsServerModel {
	
	
	// --------------------- Predefined functions for MtempsServer ---------------------
	...
	/*
	public static function getOneSelfData($idMtempsServer) {
		$id = (int) $idMtempsServer;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new MtempsServerModel();
	}
	//*/
