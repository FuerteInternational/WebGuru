 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/search_websites
 * @author       Ondrej Rafaj ()
...
class SearchWebsitesModel extends BaseSearchWebsitesModel {
	
	
	// --------------------- Predefined functions for SearchWebsites ---------------------
	...
	/*
	public static function getOneSelfData($idSearchWebsites) {
		$id = (int) $idSearchWebsites;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new SearchWebsitesModel();
	}
	//*/
