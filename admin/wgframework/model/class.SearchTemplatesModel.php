 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/search_templates
 * @author       Ondrej Rafaj ()
...
class SearchTemplatesModel extends BaseSearchTemplatesModel {
	
	
	// --------------------- Predefined functions for SearchTemplates ---------------------
	...
	public static function getTemplateByIdentifier($identifier) {
		...
		else return new SearchTemplatesModel();
	}
	//*/
...
/*
	...
	/*
	public static function getOneSelfData($idSearchTemplates) {
		$id = (int) $idSearchTemplates;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
