 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/comments_articles
 * @author       Ondrej Rafaj ()
...
class CommentsArticlesModel extends BaseCommentsArticlesModel {
	
	
	// --------------------- Predefined functions for CommentsArticles ---------------------
	...
	/*
	public static function getOneSelfData($idCommentsArticles) {
		$id = (int) $idCommentsArticles;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new CommentsArticlesModel();
	}
	//*/
