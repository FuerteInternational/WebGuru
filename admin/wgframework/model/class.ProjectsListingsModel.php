 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/projects_listings
 * @author       Ondrej Rafaj ()
...
class ProjectsListingsModel extends BaseProjectsListingsModel {
	
	
	// --------------------- Predefined functions for ProjectsListings ---------------------
	...
	/*
	public static function getOneSelfData($idProjectsListings) {
		$id = (int) $idProjectsListings;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new ProjectsListingsModel();
	}
	//*/
