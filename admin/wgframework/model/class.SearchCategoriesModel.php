 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/search_categories
 * @author       Ondrej Rafaj ()
...
class SearchCategoriesModel extends BaseSearchCategoriesModel {
	
	
	// --------------------- Predefined functions for SearchCategories ---------------------
	...
	/*
	public static function getOneSelfData($idSearchCategories) {
		$id = (int) $idSearchCategories;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new SearchCategoriesModel();
	}
	//*/
