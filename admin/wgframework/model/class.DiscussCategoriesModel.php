 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/discuss_categories
 * @author       Ondrej Rafaj ()
...
class DiscussCategoriesModel extends BaseDiscussCategoriesModel {
	
	
	// --------------------- Predefined functions for DiscussCategories ---------------------
	...
	/*
	public static function getOneSelfData($idDiscussCategories) {
		$id = (int) $idDiscussCategories;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new DiscussCategoriesModel();
	}
	//*/
