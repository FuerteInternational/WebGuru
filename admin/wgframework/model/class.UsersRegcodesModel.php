 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/users_regcodes
 * @author       Ondrej Rafaj ()
...
class UsersRegcodesModel extends BaseUsersRegcodesModel {
	
	
	// --------------------- Predefined functions for UsersRegcodes ---------------------
	...
	/*
	public static function getOneSelfData($idUsersRegcodes) {
		$id = (int) $idUsersRegcodes;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new UsersRegcodesModel();
	}
	//*/
