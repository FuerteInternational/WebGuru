 ...
 * 
 * @package      WebGuru3
 * @subpackage   wgframework/model/poll_frontends
 * @author       Ondrej Rafaj ()
...
class PollFrontendsModel extends BasePollFrontendsModel {
	
	
	// --------------------- Predefined functions for PollFrontends ---------------------
	...
	/*
	public static function getOneSelfData($idPollFrontends) {
		$id = (int) $idPollFrontends;
		if ((bool) $id) {
			$conn = new wgConnector();
			$conn->where(parent::COL_ID, $id);
			$conn->limit(1);
		...
		else return new PollFrontendsModel();
	}
	//*/
