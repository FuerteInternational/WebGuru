...
class actionsUsers extends actions {
	...
	public function __construct($parameters) {
		...
		$func['dostatsjob'] = array('doStatsJob', 'stats');
	...
	private function _doSettingsJob($parameters) {
		...
		$class = new usersActionsSettings();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doTemplatesJob($parameters) {
		...
		$class = new usersActionsTemplates();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	/**
	 * This is the _doStatsJob function description
	 *
	 * @name _doStatsJob
	 * @author Ondra Rafaj
	 * @param array $parameters Params from url
	 * @return bool Success
	 */
	private function _doStatsJob($parameters) {
		require(wgPaths::getModulePath().'actions/class.stats.php');
		$class = new usersActionsStats();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
