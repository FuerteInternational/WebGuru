<?php
/**
 * Action class for module Code Snippets
 * 
 * @package      WebGuru3
...
class actionsCodesnippets extends actions {
	...
	public function __construct($parameters) {
		...
		$func['dosnippetsjob'] = array('doSnippetsJob', 'snippets');
	...
	private function _doIndexJob($parameters) {
		...
		$class = new codesnippetsActionsIndex();
	...
	private function _doCategoriesJob($parameters) {
		...
		$class = new codesnippetsActionsCategories();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	/**
	 * This is the _doSnippetsJob function description
	 *
	 * @name _doSnippetsJob
	 * @author Ondrej Rafaj
	 * @param array $parameters Params from url
	 * @return bool Success
	 */
	private function _doSnippetsJob($parameters) {
		require(wgPaths::getModulePath().'actions/class.snippets.php');
		$class = new codesnippetsActionsSnippets();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doTemplatesJob($parameters) {
		...
		$class = new codesnippetsActionsTemplates();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
