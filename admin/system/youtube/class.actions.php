<?php
/**
 * Action class for module Youtube
 * 
 * @package      WebGuru3
...
class actionsYoutube extends actions {
	...
	private function _doTemplatesJob($parameters) {
		...
		$class = new youtubeActionsTemplates();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doSettingsJob($parameters) {
		...
		$class = new youtubeActionsSettings();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
