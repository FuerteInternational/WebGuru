 ...
 * @since        19. May 2009
 */
final class edittranslationstranslationsActionsLanguages extends BaseActions {
	/**
	...
	private static function doSaveLanguagesTranslations() {
		$save = array();
		$save['system_languages_id'] = (int) wgLang::getLanguageId();
		$save['definition'] = (string) wgPost::getValue('definition');
		$save['key'] = (string) wgPost::getValue('langkey');
		if ((bool) wgPost::getValue('defid')) {
			$save['where'] = wgPost::getValue('defid');
			$id = (int) $save['where'];
			self::$_par['edit'] = wgPost::getValue('edit');
			return (bool) LanguagesTranslationsModel::doUpdate($save);
		}
		else {
			$save['languages_definitions_id'] = (int) wgPost::getValue('edit');
