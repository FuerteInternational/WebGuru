 ...
 */
final class indexpagesActionsPages extends BaseActions {
	/**
	...
	private function _init() {
		...
		if ((bool) wgGet::getValue('moveup') || (bool) wgGet::getValue('movedown')) {
			if ((bool) wgGet::getValue('moveup')) $ok = (bool) self::doMovePages(1, wgGet::getValue('moveup'));
			if ((bool) wgGet::getValue('movedown')) $ok = (bool) self::doMovePages(-1, wgGet::getValue('movedown'));
			if ($ok) wgError::add('moved', 2);
			else wgError::add('cantmove');
			//$ok = (bool) self::doDeletePages(wgGet::getValue('delete'));
		}
		return $ok;
	}
	...
	private static function doMovePages($how, $idp) {
		$id = (int) $idp;
		if ($how > 0) return PagesModel::movePageUp($idp);
		else return PagesModel::movePageDown($idp);
	}
