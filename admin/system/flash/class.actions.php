<?php
/**
 * Action class for module Flash
 * 
 * @package      WebGuru3
 * @subpackage   modules/flash/
 * @author       Ondrej Rafaj
...
class actionsFlash extends actions {
	...
	private function _doTemplatesJob($parameters) {
		...
		$class = new flashActionsTemplates();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doSettingsJob($parameters) {
		...
		$class = new flashActionsSettings();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
