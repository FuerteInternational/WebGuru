<?php
/**
 * Action class for module MotoCatalogue
 * 
 * @package      WebGuru3
...
class actionsMotocatalogue extends actions {
	...
	private function _doIndexJob($parameters) {
		...
		$class = new motocatalogueActionsIndex();
	...
	private function _doCategoriesJob($parameters) {
		...
		$class = new motocatalogueActionsCategories();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doItemsJob($parameters) {
		...
		$class = new motocatalogueActionsItems();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
	...
	private function _doTemplatesJob($parameters) {
		...
		$class = new motocatalogueActionsTemplates();
		if ((bool) $class->init()) { wgError::add('actionok', 2);
			return true;
		}
		else { wgError::add('actionfailed');
