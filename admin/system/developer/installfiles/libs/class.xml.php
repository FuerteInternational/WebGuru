...
//-------------------------------------------------------------------
	...
	@access Public.
	*/
	public static function xml_to_array($arg_str_xml)
	{
        $parser = xml_parser_create();
		xml_parse_into_struct($parser, $arg_str_xml, $arr_raw_xml);
		$arr_out = array();
		self::prv_xml_to_array($arr_raw_xml, $arr_out);
		return $arr_out;
...
//-----------------------------------------------------------------------------	
	...
	@access Private.
	*/
	private static function prv_xml_to_array(&$arg_tags, &$arg_current_tag)
	{
		while(list(, $arr_tag) = each($arg_tags))
		{
			if($arr_tag['level'] > 1)
			{
				...
				{
					self::prv_xml_to_array($arg_tags, $arg_current_tag[$arr_tag['attributes']['KEY']]);
				} 
...
//-------------------------------------------------------------------
	...
	@access Private.
	*/
	private static function pad($arg_int_pad_number=0, $arg_str_pad="")
	{
...
//-------------------------------------------------------------------
	...
	@access Private.
	*/
	private static function prv_array_to_xml($arg_arr_array, $arg_int_pad_number=0, $arg_str_pad="")
	{
		$str_xml = "";
		while(list($k, $v) = each($arg_arr_array)){
			$str_xml .= self::pad($arg_int_pad_number, $arg_str_pad) . "<a key=\"" . htmlspecialchars($k) . "\">";
			if(is_array($v)){
				$str_xml .= self::prv_array_to_xml($v, $arg_int_pad_number+1, $arg_str_pad);
			}else{
				$str_xml .= self::pad($arg_int_pad_number+1, $arg_str_pad) . htmlspecialchars($v);
			}
			$str_xml .= self::pad($arg_int_pad_number, $arg_str_pad) . "</a>";
		}
		return $str_xml;
...
//-------------------------------------------------------------------
	...
	@access Public.
	*/
	public static function array_to_xml($arg_arr_array, $arg_str_operation_name="response", $arg_str_pad="")
	{
		if(!is_array($arg_arr_array))return false;
		$str_xml = "<$arg_str_operation_name>";
		$str_xml .= self::prv_array_to_xml($arg_arr_array, 1, $arg_str_pad);
